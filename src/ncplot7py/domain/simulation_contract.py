...
def validate_pose_request(payload: Any, profiles: Mapping[str, MachineConfig]) -> None:
    ...
    try:
        ...
        for entry in entries:
            ...
            if config.simulation["modelId"] != "MILL_DEMO":
                raise SimulationContractError(
                    "POSE_CONTRACT_UNSUPPORTED", "Only MILL_DEMO pose output is implemented", channel,
                )
            if any(tool["reference"] != "millingTip" for tool in tools):
                raise SimulationContractError(
                    "POSE_CONTRACT_UNSUPPORTED", "MILL_DEMO supports millingTip tools only", channel,
                )
